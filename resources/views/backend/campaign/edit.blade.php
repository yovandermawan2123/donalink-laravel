                    ...
                    <div class="row">
                        <div class="col-7">
                            <div class="form-group">
                                <label>Name</label>
                                <input id="name" type="text" value="{{ $campaign->name }}" name="name"
                                    class="form-control">
                            </div>
                        </div>
                        <div class="col-5">
                            <div class="form-group">
                                <label>Slug</label>
                                <input id="slug" type="text" name="slug" value="{{ $campaign->slug }}"
                        ...
                        <div class="col-4">
                            <div class="form-group">
                                <label>Target</label>
                                <input id="target" type="text" value="{{ $campaign->target }}"
                                    oninput="formatCurrency(this)" name="target" class="form-control">
                            </div>
                        </div>
                        ...
                        <div class="col-5">
                            <div class="form-group">
                                <label>Image</label>
                                <input type="hidden" name="image" value="{{ $campaign->image }}">
                                <input type="file" name="image" class="form-control">
                            </div>
                        </div>
                        <div class="col-8">
                            <div class="form-group">
                                <img src="../images/campaigns/{{ $campaign->image }}" alt="" width="200px">
                            </div>
                        </div>
...
@push('scripts')
    <script>
        ...
        $(document).ready(function() {
            var target = $('#target').val().replace(/\B(?=(\d{3})+(?!\d))/g, '.')
            $('#target').val(target);
        });
    </script>
@endpush
